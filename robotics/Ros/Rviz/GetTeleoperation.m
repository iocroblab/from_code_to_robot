...
%   MODE : string with latest teleoperation mode ("" if none yet)
    ...
    try
        msg = receive(subTwist, 0.001);  % 1 ms timeout
        lastV6 = [ double(msg.linear.x);  double(msg.linear.y);  double(msg.linear.z); ...
    ...
    try
        m = receive(subMode, 0.001);     % 1 ms timeout
        if ~isempty(m.data)
            lastMode = string(m.data);
        end
    catch
