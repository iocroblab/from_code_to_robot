function q = GetJointEfforts(order, timeout)
% GetJointStates Return joint efforts as [6x1] in a specified joint order.
%
...
% - Reorders efforts to match `order`, independent of /joint_states.name order.
    ...
    tau   = double(msg.effort);
    if numel(names) < 6 || numel(tau) < 6
