...
inner = strjoin({ ...
    sprintf('cd %s', wsPath), ...
    sprintf('source /opt/ros/jazzy/setup.bash'),...
...
if opts.Docker
    if ispc
        ...
        % Ubuntu host
        if opts.Detach
            launcher = sprintf('docker exec -d %s bash -lc %s', opts.DockerName, innerQuoted);
        else
            % if innerQuoted accidentally contains bash -lc "...", unwrap it
            innerQuoted = regexprep(innerQuoted, '^bash -lc\s+"(.*)"$', '$1');

            launcher = sprintf(['env -u LD_LIBRARY_PATH gnome-terminal -- bash -lc ', ...
                '"docker exec -it %s bash -lc ''%s''; exec bash" &'], ...
                opts.DockerName, innerQuoted);
        ...
        switch a
            ...
            case 'simulation'
                if strcmpi(ur, 'threelink')
                    base = sprintf('ros2 launch threelink_manipulator spawn_robot.launch.py');
                else
            ...
            case 'safety_nodes'
                if strcmpi(ur, 'threelink')
                    base = sprintf('ros2 run safety_nodes safety_nullifier num_joints:=3');
                else
                    base = sprintf('ros2 run safety_nodes safety_nullifier');
                end
                if ~isempty(carg), cmd = sprintf('%s %s', base, carg); else, cmd = base; end
