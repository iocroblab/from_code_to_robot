classdef PickPlaceDiscreteEnv < rl.env.MATLABEnvironment
    ...
    properties (Access = public)
        ...
        % Penalty for invalid actions (e.g., trying to pick/place when not allowed)
        penalty_invalid_action = -5

        % Penalty for random moves (discourages unnecessary movement)
        penalty_random_move = -0.1

        % Reward for moving robot to an object
        reward_robot_move_to_obj = 0.5

        % Reward for correct pick action
        reward_correct_pick = 1

        % Reward for correct place action (set to 0, as reward is added for correct position)
        reward_correct_place = 0 % Already included in correct position reward

        % Reward for moving robot (with object) to target location
        reward_robot_move_to_obj_targ = 1.5

        % Penalty for incorrect pick action
        reward_incorrect_pick = -0.5

        % Penalty for incorrect place action
