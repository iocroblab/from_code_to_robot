...
After the basic ideas of translation and rotations, we will combine them into homogeneous transform matrices, which will be the basis for the next tutorial. 
...
If the Trace if the matrix (sum of diagonal elements) is less or equal than 0. The computations need to be adjusted, otherwise you might end up with imaginary numbers for w or divide by 0 while calculating  x/y/z. 


Identify the greatest diagonal element. 
...
We can use functions to create a homogeneous transform matrices:
