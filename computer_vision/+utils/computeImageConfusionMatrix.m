function confusionMat = computeImageConfusionMatrix(bboxes_pred, scores, labels_pred, ...
    bboxes_gt, labels_gt, classNames, scoreThreshold, iouFunc, iouThreshold)
...
% Calculate the number of classes (including one extra for background)
...
elseif size(bboxes_gt, 1) ~= 0 && size(bboxes_pred, 1) == 0
    for i = 1:size(bboxes_gt, 1)
        ...
        % Count as false negative (missed a ground-truth object)
        confusionMat(bg_class, gt_class_idx) = confusionMat(bg_class, gt_class_idx) + 1;
    end
    return
    % Case 3: No predictions and no ground-truth boxes
elseif size(bboxes_gt, 1) == 0 && size(bboxes_pred, 1) == 0
    % True negative: background correctly detected
    confusionMat(bg_class, bg_class) = confusionMat(bg_class, bg_class) + 1;
    return
end


% Compute IoU for all predicted vs. ground-truth box pairs
overlaps = []; % Each row: [IoU, pred_index, gt_index]
for i = 1:size(bboxes_gt, 1)
...
% Sort overlaps by descending IoU
...
for i = 1:size(overlaps,1)
    ...
    if overlaps(i,1) < iouThreshold
        % Prediction is not close enough to any GT: false positive
        confusionMat(pred_class_idx, bg_class) = confusionMat(pred_class_idx, bg_class) + 1;
        continue
    end
    ...
    % Correct match: update confusion matrix
    confusionMat(pred_class_idx, gt_class_idx) = confusionMat(pred_class_idx, gt_class_idx) + 1;

end

% Handle unmatched ground-truth boxes (missed detections)
