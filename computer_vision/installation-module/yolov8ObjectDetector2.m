%yolov8ObjectDetector2 Detect objects using YOLO v8 deep learning detector.
%
% detector = yolov8ObjectDetector2() loads a smaller version of YOLO v8
% object detector trained to detect 80 object classes from the COCO
% dataset.
%
% detector = yolov8ObjectDetector2(detectorName) loads a pretrained YOLO v8
% object detector specified by detectorName. detectorName must be either
% 'yolov8n', 'yolov8s', 'yolov8m', 'yolov8l', or 'yolov8x'.
%
% detector = yolov8ObjectDetector2(network, classNames) configures a
...
% ----------------------------
% [...] = yolov8ObjectDetector2(..., Name=Value) specifies additional
...
%
% yolov8ObjectDetector2 properties:
...
% -----------------------------------------------------------
% % Load the pretrained detector.
% detector = yolov8ObjectDetector2('yolov8m');
%
% % Read test image.
% I = imread('highway.png');
%
...
% detector = yolov8ObjectDetector2('yolov8s',classes,InputSize = imageSize);
%
...
classdef yolov8ObjectDetector2 < vision.internal.detector.ObjectDetector
    ...
    methods
        function this = yolov8ObjectDetector2(detectorInp,classNames,options)
            arguments
            ...
            params = yolov8ObjectDetector2.parsePretrainedDetectorInputs(detectorInp,classNames,options);
    ...
    methods
        function varargout = detect(detector, I, options)
            ...
            % detector = yolov8ObjectDetector2("yolov8s");
            %
            % % Read test image.
            % I = imread("kobi.png");
            %
            ...
            arguments
                ...
                options.Threshold (1,1) {yolov8ObjectDetector2.checkThreshold(options.Threshold, 'yolov8ObjectDetector2')} = 0.25
                options.SelectStrongest (1,1) {vision.internal.inputValidation.validateLogical(options.SelectStrongest, 'SelectStrongest')} = true
                options.MiniBatchSize (1,1) {vision.internal.cnn.validation.checkMiniBatchSize(options.MiniBatchSize, 'yolov8ObjectDetector2')} = 128
                ...
                options.NMSThreshold (1,1) {mustBeNumeric} = 0.45
            end
            ...
            params.NMSThreshold             = single(options.NMSThreshold);
    ...
    methods(Static, Hidden)
        function this = loadobj(s)
            try
                vision.internal.requiresNeuralToolbox(mfilename);
                this = yolov8ObjectDetector2("uninitialized");
...
function [Ipreprocessed,info] = iPreprocessForDetect(I, executionEnvironment, networkInputSize, autoResize, castToGpuArray)
    ...
    if autoResize
        Ipreprocessed = yolov8ObjectDetector2.preprocessInput(I, info.PreprocessedImageSize);
