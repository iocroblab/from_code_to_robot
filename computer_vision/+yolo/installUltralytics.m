...
tic
[status, cmdout] = system('"' + pyInterpreter + '" -m pip install ultralytics==8.2.66');
if status==0
    disp('Done')
else
    disp(cmdout)
...
[status, cmdout] =  system('"' + pyInterpreter + '" -m pip install dill==0.3.8');
if status==0
    disp('Done')
else
    disp(cmdout)
...
[status, cmdout] = system('"' + pyInterpreter + '" -m pip install onnx==1.16.1');
if status==0
    disp('Done')
else
    disp(cmdout)
...
[status, cmdout] = system('"' + pyInterpreter + '" -m pip install tensorboard');
if status==0
    disp('Done')
else
    disp(cmdout)
...
[status, cmdout] = system('"' + pyInterpreter + '" -m yolo settings tensorboard=True');
if status==0
    disp('Done')
else
    disp(cmdout)
